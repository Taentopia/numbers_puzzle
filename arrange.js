...
s+='<p id=fldStatus class=capt2></p>';s+='<br/>';s+='<div id=board style="text-align:center; margin:auto;">';s+='</div>';s+='</div>';s+='<p>&nbsp;</p>';s+='<div style="text-align:left;">';s+='<b>Instructions:</b>';s+='<ol>';s+='<li>Sie brauchen nur ein Puzzle zu lösen.</li>';s+='<li>Nicht das Level(3) verändern oder ein neues Spiel starten, Ihre Werte gehen sonst verloren.</li>';s+='<li>Zahlen können durch Klicken verschoben werden.</li>';s+='<li>Wechseln Sie nach Beendigung zurück in den Umfrage-Tab des Browsers.</li>';s+='</ol>';s+='<p>&nbsp;</p>';s+='</div>';s+=optPopHTML();document.write(s);loadBoard(parseInt(document.getElementById('level').value));startGame();}
